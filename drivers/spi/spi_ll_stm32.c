...
{
	if (LL_SPI_GetTransferDirection(spi) != LL_SPI_HALF_DUPLEX_TX &&
			      ...
			      struct spi_stm32_data *data)
{
	if (cfg->fifo_enabled) {
		...
		if (LL_SPI_GetTransferDirection(cfg->spi) != LL_SPI_HALF_DUPLEX_RX) {
		...
		if (LL_SPI_GetTransferDirection(cfg->spi) != LL_SPI_HALF_DUPLEX_TX) {
...
static void spi_stm32_isr(const struct device *dev)
{
	...
	if (LL_SPI_GetTransferDirection(spi) == LL_SPI_FULL_DUPLEX) {
		if (!spi_stm32_transfer_ongoing(data)) {
			spi_stm32_complete(dev, err);
		}
	} else if (LL_SPI_GetTransferDirection(spi) == LL_SPI_HALF_DUPLEX_TX) {
		      ...
		      void *userdata)
{
	...
	while (data->ctx.rx_len > 0 || data->ctx.tx_len > 0) {
		...
		} else if (LL_SPI_GetTransferDirection(spi) == LL_SPI_HALF_DUPLEX_RX) {
